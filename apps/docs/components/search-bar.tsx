...
const SearchBar = () => {
  ...
  const { data: results, isRefetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`http://localhost:3000/api/search?query=${debouncedSearchTerm}`)
  ...
  const displayedResults = isRefetching ? [] : results
        ...
        if (exactIndex !== -1) {
          ...
          if (token.length > searchWord.length) {
            ...
            highlightedToken = (
              <>
                {prefix}
                <mark className="bg-brand-400/20 text-brand-400 px-0.5 rounded">{match}</mark>
                {suffix}
              </>
            )
          }
  ...
  const renderMarkdownContent = (content: string) => {
    const patterns = [
      ...
      {
        regex: /(?<=\s)_([^_]+)_(?=\s)/g,
        render: (_: string, text: string) => <i className="text-muted-light italic">{text}</i>,
      },
    ]
            ...
            >
              {displayedResults.map((result: any, index: number) => (
                ...
                >
                  ...
                  >
                    {highlightMatches(result.metadata?.documentTitle || "", searchTerm)}
                  </h3>
                  ...
                  <p className="text-sm text-zinc-300 mt-2 leading-relaxed">
                    {result.metadata?.content &&
                      renderMarkdownContent(
                        getContextAroundMatch(result.metadata.content, searchTerm),
                      ).map((element, index) =>
