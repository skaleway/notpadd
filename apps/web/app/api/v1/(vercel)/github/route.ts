...
export async function POST(req: Request) {
  try {
    ...
    if (!space.ghFinedGrainedToken || !space.ghUsername || !space.ghRepository) {
    ...
    const do_not_edit_this_file_content = Buffer.from(currentFileData.content).toString("base64")
