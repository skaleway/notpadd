...
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    ...
    const limit: number = Math.min(100, Math.max(1, parseInt(headers.get("limit") || "20", 10)))
    ...
    const validateVisibilityFlags = (): void => {
      ...
      if (all && publishOnly) {
        errors.push("Cannot fetch both all content and published-only content simultaneously")
      }
    ...
    const endTime: number = performance.now()
    console.error(`Error fetching articles (${Math.round(endTime - startTime)}ms)`, error)
